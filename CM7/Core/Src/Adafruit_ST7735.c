...
const struct st7735_cmd st7735_gmctrp1={
	...
	.delay=0,
	.cnt=16,
	.data={0x02, 0x1c, 0x07, 0x12, 0x37, 0x32, 0x29, 0x2d, 0x29, 0x25, 0x2B, 0x39, 0x00, 0x01, 0x03, 0x10}
};
...
const struct st7735_cmd st7735_gmctrn1={
	...
	.delay=0,
	.cnt=16,
	.data={0x03, 0x1d, 0x07, 0x06, 0x2E, 0x2C, 0x29, 0x2D, 0x2E, 0x2E, 0x37, 0x3F, 0x00, 0x00, 0x02, 0x10}
};
...
/**
  ...
  * @retval HAL status
  */
HAL_StatusTypeDef QSPI_WaitFlagStateUntilTimeout(QSPI_HandleTypeDef *hqspi, uint32_t Flag, FlagStatus State, uint32_t Tickstart, uint32_t Timeout)
{
...
//	__IO uint32_t *data_reg=&(hqspi.Instance->DR);;
	...
	// Check our busy
	status = QSPI_WaitFlagStateUntilTimeout(&hqspi, QSPI_FLAG_BUSY, RESET, tickstart, 10000);
	if (status!=HAL_OK) {
		return HAL_BUSY;
	}
	// Now we need to set up the QSPI write command
	WRITE_REG(hqspi.Instance->DLR,160*128*2-1);
	WRITE_REG(hqspi.Instance->CCR,QSPI_DDR_MODE_DISABLE|QSPI_SIOO_INST_ONLY_FIRST_CMD|QSPI_ADDRESS_NONE|QSPI_INSTRUCTION_NONE|QSPI_ALTERNATE_BYTES_NONE|QSPI_DATA_1_LINE);
	...
	// Wait until finished
	status = QSPI_WaitFlagStateUntilTimeout(&hqspi, QSPI_FLAG_TC, SET, tickstart, 500);
	if (status == HAL_OK) {
...
HAL_StatusTypeDef qspi_write_cmd(const uint8_t cmd)
{
	...
	WRITE_REG(hqspi.Instance->CCR,QSPI_SIOO_INST_ONLY_FIRST_CMD|QSPI_ADDRESS_NONE|QSPI_INSTRUCTION_1_LINE|QSPI_ALTERNATE_BYTES_NONE|QSPI_DATA_NONE|cmd);
	status = QSPI_WaitFlagStateUntilTimeout(&hqspi, QSPI_FLAG_TC, SET, tickstart, 500);
	if (status == HAL_OK) {
		__HAL_QSPI_CLEAR_FLAG(&hqspi, QSPI_FLAG_TC);
	}
...
HAL_StatusTypeDef qspi_write_data(const uint8_t *data, const uint8_t cnt)
{
	...
	WRITE_REG(hqspi.Instance->CCR,QSPI_DDR_MODE_DISABLE|QSPI_ADDRESS_NONE|QSPI_INSTRUCTION_NONE|QSPI_ALTERNATE_BYTES_NONE|QSPI_DATA_1_LINE);
	// Write out data
	// CS is managed in hardware
	for (i=0;i<cnt;i++) {
